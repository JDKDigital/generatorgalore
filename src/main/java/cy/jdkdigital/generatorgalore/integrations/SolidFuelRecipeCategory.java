...
public class SolidFuelRecipeCategory implements IRecipeCategory<SolidFuelRecipe>
{
    ...
    public SolidFuelRecipeCategory(IGuiHelper guiHelper, GeneratorObject generator) {
        ...
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM, new ItemStack(ForgeRegistries.ITEMS.getValue(new ResourceLocation(GeneratorGalore.MODID, this.generator.getId().getPath() + "_generator"))));
    }
    ...
    public @NotNull Component getTitle() {
        return new TranslatableComponent(GeneratorGalore.MODID + ".recipe.solid_fuel");
    }
    ...
    @Override
    public void draw(@NotNull SolidFuelRecipe recipe, @NotNull IRecipeSlotsView recipeSlotsView, @NotNull PoseStack poseStack, double mouseX, double mouseY) {
        Minecraft minecraft = Minecraft.getInstance();
        minecraft.font.draw(poseStack, "Rate: " + recipe.rate() + "FE/t", 37, 14, 4210752);
        minecraft.font.draw(poseStack, "Burntime: " + recipe.burnTime(), 37, 32, 4210752);
        minecraft.font.draw(poseStack, "Total: " + (int) (recipe.rate() * recipe.burnTime()) + "FE", 37, 50, 4210752);
    }
